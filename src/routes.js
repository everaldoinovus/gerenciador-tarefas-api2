...
router.get('/setores', authMiddleware, async (req, res) => { const usuarioId = req.usuarioId; const userRole = req.role; try { let sql; let params; if (userRole === 'admin') { sql = ` SELECT s.*, 'admin' AS funcao FROM setores s ORDER BY s.nome ASC; `; params = []; } else { sql = ` SELECT s.*, us.funcao FROM setores s JOIN usuarios_setores us ON s.id = us.setor_id WHERE us.usuario_id = ? ORDER BY s.nome ASC; `; params = [usuarioId]; } const [rows] = await pool.query(sql, params); res.status(200).json(rows); } catch (error) { console.error("Erro ao listar setores:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); } });
...
router.get('/setores/:id/membros', authMiddleware, async (req, res) => { const { id: setorId } = req.params; const usuarioId = req.usuarioId; try { const [permRows] = await pool.query('SELECT 1 FROM usuarios_setores WHERE usuario_id = ? AND setor_id = ?', [usuarioId, setorId]); if (permRows.length === 0) return res.status(403).json({ error: 'Acesso negado. Você não tem permissão para ver os membros deste setor.' }); const sql = ` SELECT u.id, u.nome, u.email, us.funcao FROM usuarios u JOIN usuarios_setores us ON u.id = us.usuario_id WHERE us.setor_id = ? ORDER BY u.nome, u.email `; const [members] = await pool.query(sql, [setorId]); res.status(200).json(members); } catch (error) { console.error("Erro ao listar membros do setor:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); } });
...
router.get('/users/me', authMiddleware, async (req, res) => { try { const [rows] = await pool.query( 'SELECT id, email, nome, role FROM usuarios WHERE id = ?', [req.usuarioId] ); if (rows.length === 0) { return res.status(404).json({ error: 'Usuário não encontrado.' }); } res.json(rows[0]); } catch (error) { console.error("Erro ao buscar dados do usuário:", error); res.status(500).json({ error: 'Erro ao buscar dados do usuário.' }); } });
router.put('/users/update-profile', authMiddleware, async (req, res) => { const { nome } = req.body; const usuarioId = req.usuarioId; if (!nome || nome.trim() === '') { return res.status(400).json({ error: 'O nome é obrigatório.' }); } try { await pool.query('UPDATE usuarios SET nome = ? WHERE id = ?', [nome.trim(), usuarioId]); res.status(200).json({ message: 'Perfil atualizado com sucesso!' }); } catch (error) { console.error("Erro ao atualizar perfil:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); } });
...
router.get('/users', authMiddleware, checkGlobalRole(['admin']), async (req, res) => { try { const [users] = await pool.query('SELECT id, email, nome, role, status FROM usuarios ORDER BY email ASC'); res.status(200).json(users); } catch (error) { console.error("Erro ao listar usuários:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); } });
...
router.put('/users/change-password', authMiddleware, async (req, res) => { const { senhaAtual, novaSenha } = req.body; const usuarioId = req.usuarioId; if (!senhaAtual || !novaSenha) { return res.status(400).json({ error: 'A senha atual e a nova senha são obrigatórias.' }); } if (novaSenha.length < 4) { return res.status(400).json({ error: 'A nova senha deve ter pelo menos 4 caracteres.' }); } let connection; try { connection = await pool.getConnection(); const [rows] = await connection.query('SELECT senha_hash FROM usuarios WHERE id = ?', [usuarioId]); if (rows.length === 0) { return res.status(404).json({ error: 'Usuário não encontrado.' }); } const usuario = rows[0]; const senhaCorreta = await bcrypt.compare(senhaAtual, usuario.senha_hash); if (!senhaCorreta) { return res.status(401).json({ error: 'A senha atual está incorreta.' }); } const novaSenhaHash = await bcrypt.hash(novaSenha, 10); await connection.query('UPDATE usuarios SET senha_hash = ? WHERE id = ?', [novaSenhaHash, usuarioId]); res.status(200).json({ message: 'Senha alterada com sucesso!' }); } catch (error) { console.error("Erro ao alterar senha:", error); res.status(500).json({ error: 'Erro interno do servidor ao alterar a senha.' }); } finally { if (connection) { connection.release(); } } });

// ===================================================================
// ROTAS DE ARQUIVAMENTO DE TAREFAS (AGORA INCLUÍDAS)
// ===================================================================

router.get('/setores/:id/tarefas/arquivadas', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const sql = `
            SELECT t.id, t.descricao, t.data_prevista_conclusao, u.nome as responsavel_nome
            FROM tarefas t
            LEFT JOIN usuarios u ON t.responsavel_id = u.id
            WHERE t.setor_id = ? AND t.estado = 'arquivado'
            ORDER BY t.data_prevista_conclusao DESC
        `;
        const [rows] = await pool.query(sql, [setorId]);
...
router.put('/tarefas/:id/archive', authMiddleware, async (req, res) => {
    const { id: tarefaId } = req.params;
    try {
        const [result] = await pool.query("UPDATE tarefas SET estado = 'arquivado' WHERE id = ?", [tarefaId]);
...
router.put('/tarefas/:id/unarchive', authMiddleware, async (req, res) => {
    const { id: tarefaId } = req.params;
    try {
        const [result] = await pool.query("UPDATE tarefas SET estado = 'ativo' WHERE id = ?", [tarefaId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tarefa não encontrada.' });
        }
        res.status(200).json({ message: 'Tarefa restaurada com sucesso.' });
    } catch (error) {
...
router.delete('/tarefas/:id/permanent', authMiddleware, async (req, res) => {
    const { id: tarefaId } = req.params;
    try {
        await pool.query('DELETE FROM historico_status_tarefas WHERE tarefa_id = ?', [tarefaId]);
        const [result] = await pool.query('DELETE FROM tarefas WHERE id = ?', [tarefaId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tarefa não encontrada.' });
        }
        res.status(200).json({ message: 'Tarefa deletada permanentemente.' });
    } catch (error) {
...
const checkOwnership = (req, res, next) => { if (req.userRole !== 'dono' && req.role !== 'admin') { return res.status(403).json({ error: 'Acesso negado: privilégios de dono ou admin necessários.' }); } next(); };

const checkSectorOwnershipForDeletion = async (req, res, next) => { const setorId = req.params.id; const usuarioId = req.usuarioId; const userRole = req.role; if (userRole === 'admin') { return next(); } try { const [rows] = await pool.query('SELECT funcao FROM usuarios_setores WHERE usuario_id = ? AND setor_id = ?', [usuarioId, setorId]); if (rows.length === 0 || rows[0].funcao !== 'dono') { return res.status(403).json({ error: 'Acesso negado. Apenas o dono do setor ou um admin podem deletar.' }); } next(); } catch (error) { console.error("Erro no middleware de verificação de posse do setor:", error); res.status(500).json({ error: 'Erro no servidor ao verificar permissão para deletar setor.' }); } };
...
//Rotina com inclusão de nome do usuário
router.get('/setores/:id/membros', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // Query agora busca também o nome do usuário
        const sql = `
            SELECT u.id, u.nome, u.email, us.funcao 
            FROM usuarios u 
    ...
    try {
        const sql = `
            SELECT 
                ...
                u.nome AS responsavel_nome,     -- << MUDANÇA AQUI
                u.email AS responsavel_email,   -- Mantido para referência, se necessário
        ...
        // A lógica de processamento de atraso permanece a mesma
        ...
        const tarefasProcessadas = rows.map(tarefa => {
            ...
            if (tarefa.tempo_maximo_dias > 0 && tarefa.data_entrada_status_atual) {
                ...
                if (diffDays > tarefa.tempo_maximo_dias) { esta_atrasado_sla = true; }
            }
            ...
            if (tarefa.data_prevista_conclusao && !tarefa.data_finalizacao) {
                ...
                if (hoje > dataPrevista) { prazo_estourado = true; }
            }
    ...
    try {
        ...
        if (updates.status_id && updates.status_id !== statusAtualId) {
            ...
            if (tarefaAtual.tarefa_pai_id) {
                const [acaoOrigemRows] = await connection.query( `SELECT a.* FROM acoes_automacao a JOIN regras_automacao r ON a.regra_id = r.id WHERE r.setor_origem_id = (SELECT setor_id FROM tarefas WHERE id = ?) AND a.setor_destino_id = ?`, [tarefaAtual.tarefa_pai_id, tarefaAtual.setor_id] );
                
                if (acaoOrigemRows.length > 0) {
                    const acao = acaoOrigemRows[0];
                    if (acao.ativar_feedback_analise) {
...
// NOVA ROTA: GET /users/me - Busca os dados do usuário logado
...
// NOVA ROTA: PUT /users/update-profile - Atualiza o nome do usuário logado
router.put('/users/update-profile', authMiddleware, async (req, res) => {
    ...
    try {
        await pool.query('UPDATE usuarios SET nome = ? WHERE id = ?', [nome.trim(), usuarioId]);
