...
const checkMembership = async (req, res, next) => {
    let setorId;
    // Prioriza o setor_id vindo do corpo da requisição (ex: criar tarefa)
    if (req.body.setor_id) {
        setorId = req.body.setor_id;
    } 
    // Se não, tenta pegar dos parâmetros da URL (ex: /setores/:id)
    else if (req.params.id) {
        const resourceId = req.params.id;
        try {
            // Se a rota for de uma tarefa, precisamos buscar o setor_id dela
            if (req.path.includes('/tarefas/')) {
                ...
                // Se for uma rota de setor, o ID no parâmetro já é o setorId
                setorId = resourceId;
            }
        } catch (e) {
    ...
    if (!setorId) return res.status(400).json({ error: 'Não foi possível determinar o setor para a verificação de permissão.' });
    ...
    try {
        ...
        req.userRole = rows[0].funcao; // Anexa a função do usuário para a próxima etapa
        next();
    } catch (error) {
...
router.get('/tarefas', authMiddleware, async (req, res) => {
    const usuarioId = req.usuarioId;
    try {
        const sql = ` SELECT t.*, s.nome AS setor, u.email AS responsavel_email FROM tarefas t JOIN setores s ON t.setor_id = s.id LEFT JOIN usuarios u ON t.responsavel_id = u.id WHERE t.setor_id IN (SELECT setor_id FROM usuarios_setores WHERE usuario_id = ?) `;
        const [rows] = await pool.query(sql, [usuarioId]);
        res.status(200).json(rows);
    } catch (error) {
...
router.put('/tarefas/:id', authMiddleware, checkMembership, async (req, res) => {
    ...
    if (setor_id) {
        try {
            ...
            if (permRows.length === 0) {
                return res.status(403).json({ error: 'Acesso negado: você não é membro do setor de destino.' });
            }
        } catch (error) {
            return res.status(500).json({ error: 'Erro ao verificar permissão no setor de destino.' });
        }
    }
    ...
    try {
        ...
        const values = [
            ...
            responsavel_id, // Permite que seja null
            setor_id ?? tarefaAtual.setor_id,
            status ?? tarefaAtual.status,
            data_prevista_conclusao ?? tarefaAtual.data_prevista_conclusao,
            data_finalizacao ?? tarefaAtual.data_finalizacao,
            notas ?? tarefaAtual.notas,
            tarefaId
        ];
...
router.get('/tarefas/:id/historico', authMiddleware, checkMembership, async (req, res) => {
    const { id: tarefaId } = req.params;
    try {
        const sql = ` SELECT h.status_anterior, h.status_novo, h.data_alteracao, u.email AS usuario_alteracao_email FROM historico_status_tarefas h JOIN usuarios u ON h.usuario_alteracao_id = u.id WHERE h.tarefa_id = ? ORDER BY h.data_alteracao ASC; `;
