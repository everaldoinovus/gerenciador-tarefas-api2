...
router.get('/setores/:id/membros', authMiddleware, checkMembership, async (req, res) => { const { id: setorId } = req.params; try { const sql = ` SELECT u.id, u.email, us.funcao FROM usuarios u JOIN usuarios_setores us ON u.id = us.usuario_id WHERE us.setor_id = ? ORDER BY u.email`; const [members] = await pool.query(sql, [setorId]); res.status(200).json(members); } catch (error) { console.error("Erro ao listar membros do setor:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); } });
...
// LISTAR TAREFAS - ATUALIZADO PARA INCLUIR O E-MAIL DO RESPONSÁVEL
...
router.delete('/tarefas/:id', authMiddleware, checkMembership, async (req, res) => {
    if (req.userRole !== 'dono') {
        return res.status(403).json({ error: 'Acesso negado. Apenas o dono pode deletar tarefas.' });
    }
    const { id: tarefaId } = req.params;
    try {
