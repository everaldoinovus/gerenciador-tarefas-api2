...
// ===============================================
// MIDDLEWARE DE PERMISSÕES REESCRITO E ROBUSTO
// ===============================================

const attachUserRole = async (req, res, next) => {
    // Determina o ID do setor a ser verificado
    let setorId;
    // Para rotas /setores/:id/... ou quando setor_id está no corpo
    if (req.params.id && req.path.includes('/setores/')) {
        setorId = req.params.id;
    } else if (req.body.setor_id) {
        setorId = req.body.setor_id;
    } else if (req.params.id) { // Para rotas /tarefas/:id/...
        try {
            const [taskRows] = await pool.query('SELECT setor_id FROM tarefas WHERE id = ?', [req.params.id]);
            if (taskRows.length > 0) {
                setorId = taskRows[0].setor_id;
            } else {
                // Se a tarefa não for encontrada, a rota principal tratará o 404.
                // Aqui, apenas seguimos em frente sem um papel definido.
                return next();
            }
        } catch (e) {
            return res.status(500).json({ error: 'Erro interno ao verificar o recurso.' });
        }
    }

    if (!setorId) {
        // Se não conseguirmos determinar um setor, apenas continuamos.
        // A lógica da rota decidirá se o acesso é permitido.
        return next();
    }
    
    // Agora que temos o setorId, verificamos a permissão
    try {
        const [rows] = await pool.query('SELECT funcao FROM usuarios_setores WHERE usuario_id = ? AND setor_id = ?', [req.usuarioId, setorId]);
        if (rows.length > 0) {
            req.userRole = rows[0].funcao; // Anexa a função do usuário
        }
        next();
    } catch (error) {
...
router.post('/setores/:id/convidar', authMiddleware, attachUserRole, async (req, res) => {
    if (req.userRole !== 'dono') return res.status(403).json({ error: 'Acesso negado: Apenas donos podem convidar.' });
...
router.delete('/setores/:id', authMiddleware, attachUserRole, async (req, res) => {
    if (req.userRole !== 'dono') return res.status(403).json({ error: 'Acesso negado: Apenas donos podem deletar setores.' });
...
router.get('/setores/:id/membros', authMiddleware, attachUserRole, async (req, res) => {
    if (!req.userRole) return res.status(403).json({ error: 'Acesso negado a este setor.' });
    const { id: setorId } = req.params; try { const sql = ` SELECT u.id, u.email, us.funcao FROM usuarios u JOIN usuarios_setores us ON u.id = us.usuario_id WHERE us.setor_id = ? ORDER BY u.email`; const [members] = await pool.query(sql, [setorId]); res.status(200).json(members); } catch (error) { console.error("Erro ao listar membros do setor:", error); res.status(500).json({ error: 'Erro interno do servidor.' }); }
});
